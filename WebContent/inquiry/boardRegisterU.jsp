...
<div class="page_header">
	 <div class="faq_header">
	       <div class="name">
	             <p class="comeText">문의게시판 수정/삭제</p>
	        </div>
	  </div>
</div>
        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
					...
					<form role="form" action="/work/inquiry/updateBoard.do" method="post">	
...
<%-- 						<input type="hidden" name="type" value='<c:out value="${cri.type}"/>'> --%>
						...
						<button id="buttons" type="submit" data-oper='modify' class="btn btn-dark" onclick="location.href=${context}/work/inquiry/updateBoard.do?bno=${dsBoard.BNO}">수정</button>	
						<button id="buttons" type="submit" data-oper='remove' class="btn btn-danger">삭제</button>
						<button id="buttons" type="submit" data-oper='list' class="btn btn-secondary" onclick="location.href='/inquiry/boardListR.jsp'">목록</button>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		$('button').on("click", function (e) {
			...
			if (operation === 'remove') {
				formObj.attr("action", "/work/inquiry/deleteBoard.do");
				
				// 만약 사용자가 'list' 버튼을 클릭한다면
			} else if (operation === 'list') {
// 				move to list : 아래 두 줄 코딩
				self.location = "${context}/work/inquiry/retrieveBoardList.do";
				return;
				
				// Page266 11.5.4 수정 페이지에서 링크 처리 소스 코딩 : 아래 두 줄
// 				formObj.attr("action", "/inquiry/boardListR").attr("method", "get");
