...
<body>
				...
				<div class="panel panel-default">
					<c:if test="${sessionScope.id != null}">
						<div class="panel-heading">
							<button id="regBtn" type="button" class="btn btn-dark pull-right">새
								글쓰기</button>
						</div>
					</c:if>
					<!-- /.panel-heading -->
					<div class="panel-body">
						<table width="100%"
							...
							<tbody>
								<c:forEach items="${dsBoardList}" var="dsBoardList"
									varStatus="courseIdx">
									<tr>
										<td class="td_1"><a class="nt"
											href="${context}/work/inquiry/retrieveBoard.do?bno=${dsBoardList.bno}">${dsBoardList.bno}</a></td>
										<td class="td_2"><a class="nt"
											href="${context}/work/inquiry/retrieveBoard.do?bno=${dsBoardList.bno}">${dsBoardList.title}</a></td>
										<td class="td_3"><c:out value="${dsBoardList.writer}" /></td>
										<td class="td_4"><fmt:formatDate pattern="yyyy-MM-dd"
												value="${dsBoardList.regdate}" /></td>
						...
						<div class="row">
							<div class="col-lg-12">
								<form id="searchForm"
									action="/work/inquiry/retrieveBoardList.do" method="get">
									...
									<select name="type">
										...
										<option value="TWC"
											...
											or 내용 or 작성자</option>
									</select> <input type="text" class="search_write" name="keyword"
										value='<c:out value="${pageMaker.cri.keyword}"/>' /> <input
										type="hidden" name="pageNum"
										value='<c:out value="${pageMaker.cri.pageNum}"/>' /> <input
										type="hidden" name="amount"
										value='<c:out value="${pageMaker.cri.amount}"/>' />
									<button id="srbtn" class="btn btn-dark">검색</button>
								</form>
							</div>
						</div>
	...
	<form id="actionForm" action="/work/inquiry/retrieveBoardList.do"
		method="get">
				...
				.ready(
						function() {
							...
							$("#regBtn").on("click", function() {
								self.location = "/work/inquiry/createBoard.do";
							});
